...
test.describe('Ticket API', () => {
    ...
    test.beforeAll(async ({ request }) => {
        studentToken = await login(request, studentEmail, 'Password@123');
        adminToken = await login(request, adminEmail, adminPassword);
    });

    test('should raise a new ticket', async ({ request }) => {
        const response = await request.post('/api/ticket/raise', {
    ...
    test('should fetch my tickets', async ({ request }) => {
        const response = await request.get('/api/ticket/my-tickets', {
            headers: { 'Authorization': `Bearer ${studentToken}` }
        });
    ...
    test('admin should fetch all tickets', async ({ request }) => {
        const response = await request.get('/api/ticket/all', {
    ...
    test('admin should update ticket status', async ({ request }) => {
        ...
        const response = await request.patch(`/api/ticket/${testTicketId}/status`, {
            headers: { 'Authorization': `Bearer ${adminToken}` },
            data: {
                status: 'In Progress',
    ...
    test('should delete a pending ticket (if applicable)', async ({ request }) => {
        ...
        const createRes = await request.post('/api/ticket/raise', {
            headers: { 'Authorization': `Bearer ${studentToken}` },
            data: {
                type: 'IT Support',
                title: 'Delete Me',
        ...
        const response = await request.delete(`/api/ticket/${tempTicketId}`, {
            headers: { 'Authorization': `Bearer ${studentToken}` },
